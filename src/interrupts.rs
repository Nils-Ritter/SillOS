...
extern "x86-interrupt" fn breakpoint_handler(stack_frame: InterruptStackFrame){
    println_error!("[EXCEPTION] Breakpoint\n{:#?}", stack_frame);
}

extern "x86-interrupt" fn double_fault_handler(
    stack_frame: InterruptStackFrame,
    _error_code: u64) -> ! {
    println_error!("[EXCEPTION] Double Fault\n{:#?}", stack_frame);
    loop {}
}
...
extern "x86-interrupt" fn page_fault_handler(stack_frame: InterruptStackFrame, error_code: PageFaultErrorCode){
    ...
    println_error!("[EXCEPTION] Page fault");
    println_error!("Accessed Address: {:?}", Cr2::read());
    println_error!("Error Code: {:?}", error_code);
    println_error!("{:?}", stack_frame);
    hlt_loop();
}
